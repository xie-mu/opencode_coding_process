...
class FixedGitHubSkillsLoader:
    ...
    def clone_skills_repo(self):
        ...
        try:
            ...
            skills_repo_url = "https://github.com/openclaw/skills"
            os.system(f"cd {self.temp_dir} && git clone {skills_repo_url} skills_repo")
            print("✅ 技能仓库克隆完成")
            return True
    ...
    def scan_skills_from_repo(self):
        ...
        for skill_folder in skills_dir.iterdir():
            if skill_folder.is_dir() and "node_modules" not in str(skill_folder) and ".git" not in str(skill_folder):
                skill_md_file = skill_folder / "SKILL.md"
                if skill_md_file.exists():
                    skill_info = {
                        "name": skill_folder.name,
                        "path": str(skill_folder),
                        "skill_file": str(skill_md_file),
                        "source": "github",
                        "created": skill_md_file.stat().st_ctime,
                        "modified": skill_md_file.stat().st_mtime
                    }
    ...
    def _count_files(self, directory: Path) -> dict:
        """统计目录文件"""
        files = {"total": 0, "py": 0, "md": 0, "txt": 0, "json": 0, "yaml": 0, "js": 0, "ts": 0, "size": 0}
    ...
    def _display_results(self, skills):
        ...
        total_size = sum(skill.get("size", 0) for skill in skills) / 1024  # KB
...
def main():
    """主函数"""
    loader = FixedGitHubSkillsLoader()
